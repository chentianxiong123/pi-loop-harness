...
>
> **权限：Hermes 专属。** CC 和 Codex 不能检索，只有 Hermes 有 recall 权限。
> 检索结果由 Hermes 整理后，通过 STATUS.md 上下文注入给 CC/Codex。
...
- "有没有关于 Y 的归档？"
- "我所有项目里用过哪些 Z 方案？"
- Hermes 接到新任务，需要先检索已有成果（**每次新任务必 recall**）

## 不要触发

- CC 或 Codex 直接调用 — 拒绝，必须通过 Hermes
