...
读账本：`runstate_get`（name=<name>, expectStage=plan；缺失则 `RESULT: NO_RUN_STATE`）。
...
3. 判定：
   - **可行** → 写 `.pi/feasibility/<name>.md`（结论 feasible + **注意点/风险清单** + 实现要点），用 `runstate_update` 更新（stage=explore, event=FEASIBLE），产出后再回归对一遍原始需求没跑偏。末行 `RESULT: FEASIBLE path=.pi/feasibility/<name>.md`。
   - **有问题** → **回归提问**：不写可行性文件，`runstate_update`（stage=ask, event=INFEASIBLE reason），`RESULT: INFEASIBLE reason=<...>`，并带 reason 回到 **环节1** 重新与用户对齐需求。
